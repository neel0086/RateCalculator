...
const BoxSearch = ({ contract }) => {
  ...
  useEffect(() => {
    ...
    loadData().catch((err) => console.error("Error reading input box file:", err))
  }, [])
  ...
  const readUploadFile = (e) => {
    ...
    if (e.target.files) {
      ...
      reader.onload = (e) => {
        ...
        replaceCompanyData(process.env.REACT_APP_INPUTBOXFILE, json).then(() => {
          setIsLoading(true)
          setTimeout(() => {
            setIsLoading(false);
        ...
        }).catch((err) => console.error("Error importing input box file:", err))
      };
                ...
                {!searchData ? "" :
                  searchData.map(function (data, index) {
                    return (
                      ...
                      <tr onClick={() => { navigate("/box_rate", { state: data }) }} className=" border-b border-opacity-5 bg-inherit  hover:bg-zinc-900 cursor-pointer">
